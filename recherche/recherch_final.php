	        ...
	        //foreach (array_slice($results,1,2) as $annonce)
	            ...
	            <div class="container-fluid">
	            	...
	            	<div class="row">
	            		<div class="col-md-10">
	                        <div class="card annonce">
								<h3 class="pull-right">
									<button onclick="ajouterFavori1(<?php echo $annonce['idTrajetTransporteur'];?>)"><i class="fa fa-heart-o pull-right" aria-hidden="true" title="Ajouter à vos favoris" style="margin-right:18px;margin-top:10px;"></i></button>
								</h3>
	                            ...
	                            <div class="content" >
	                                ...
	                                <div class="footer">
	                                   ...
	                                   <?php 
	                                   $existe_qr= "SELECT * FROM `relationenv_trans` WHERE (`idEnv`=".$_SESSION["ID"]." AND `idTrans`=".$res[0]['User_idUser'].")";
	                                   $existe_res=$connect->runQuery($existe_qr);
	                                   if(count($existe_res)==0)
	                                   {
	                                   	$accpt=false;
	                                   }else $accpt=true;
	                                   ...
	                                   ?>
	                                    ...
	                                    <div class="legend pull-right">
	                                      <button type="button" class="btn btn-default btn-sm" onclick="inviter(<?php echo $res[0]['User_idUser']?>)" >
						        			Envoyer une demande
					        			  </button> 
	                                    </div>
	                                    <?php }else;?>
		                                <div class="legend" data-toggle="modal" data-target="#modalUser1">
			                                    ...
			                                    <img src=<?php if (!empty($user[0]["PhotoProfil"])) echo '"'.$user[0]["PhotoProfil"]."'";else echo '"assets/img/default-avatar.png"';?> alt='Déposeur de lannonce' class="img-circle img-avatar" alt="Déposeur de l'annonce" class="img-circle img-avatar">
					...
					for ($p=1;$p<=$number_of_pages;$p++) {
						...
						$rqst=$rqst."&page=".$p;
					  echo '<li class="page-item"><a href="http://localhost/projet/recherche/recherch_final?'.$rqst.'">' . $p . '</a></li> ';
					}
					echo "</ul>";
	            	}?>
...
<?php if(isset($_GET["submit2"]))
			...
			<div class="content tab-pane" id="rechercheColisTab" >
	            ...
	            <div class="container-fluid" >
	            	<div class="row">
	            		<div class="col-md-10">
	                        <div class="card annonce">
								<h3 class="pull-right">
									<button onclick="ajouterFavori2(<?php echo $annonce2['idTrajetColis'];?>)"><i class="fa fa-heart-o pull-right" aria-hidden="true" title="Ajouter à vos favoris" style="margin-right:18px;margin-top:10px;"></i></button>
								</h3>
	                            ...
	                            <div class="content container-fluid">
	                                ...
	                                <div class="footer">
	                                   <?php 
	                                   $existe_qr2= "SELECT * FROM `relationenv_trans` WHERE (`idEnv`=".$env[0]["User_idUser"]." AND `idTrans`=".$_SESSION["ID"].")";
	                                   $existe_res2=$connect->runQuery($existe_qr2);
	                                   if(count($existe_res2)==0)
	                                   {
	                                   	$accpt2=false;
	                                   }else $accpt2=true;
	        	...
	        	<?php }
				echo '<ul class="pagination flex-container container-fluid">';
				for ($p2=1;$p2<=$number_of_pagesB;$p2++) {
					$rqst2=$_SERVER["QUERY_STRING"];
					$rqst2=str_replace("pageB=".$pageB,"", $rqst2);
					$rqst2=$rqst2."&pageB=".$p2;
					//echo '<ul class="pagination flex-container container-fluid">';
				  echo '<li class="page-item"><a href="http://localhost/projet/recherche/recherch_final.php?'.$rqst2.'">' . $p2 . '</a></li> ';
				}
				echo "</ul>";
				}?>
	        </div> 
		      ...
		      <div class="modal-body">
		        ...
		        <tbody>
		        	<tr>
						...
						<td><?php if ($user[0]["UserType"]==0) echo "Premium";else echo "simple";?></td>
					...
					<tr>
						...
						<td><?php echo $user[0]["Vehicule"];?></td>
									      
					</tr>
					<tr>
						<th scope="row"> Date d'inscription: </th>
						<td><?php echo $user[0]["DateInscription"];?></td>
		    ...
		    <div class="modal-content">
		      ...
		      <div class="modal-body">
		        ...
		        <tbody>
		        	<tr>
						<th scope="row">Type: </th>
						<td><?php if ($usr[0]["UserType"]==0) echo "Premium";else echo "simple";?></td>
					...
					<tr>
						<th scope="row"> Vehicule: </th>
						<td><?php echo $usr[0]["Vehicule"];?></td>
									      
					</tr>
					<tr>
						<th scope="row"> Date d'inscription: </th>
						<td><?php echo $usr[0]["DateInscription"];?></td>
...
$(document).ready(function(){
    ...
    <!--  ajax functions -->
    <script type="text/javascript">
	    function ajouterFavori1(val){
	    	//var data1 = 'idAnnonce='+val1+'&type='+val2;
			    $.ajax({
			        type: "POST",
			        url: "favorie1.php",
			        data:'idAnnonce='+val,
			        success: function(result) {
			            alert("L'annonce a été sauvegardée");
			        },
...
function inviter(val){
    $.ajax({
        ...
        success: function(result) {
            alert("L'invitaion a été envoyée");
        },
        error: function(result) {
            alert('error');
        }
